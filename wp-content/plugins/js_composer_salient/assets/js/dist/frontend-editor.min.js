...
window.vc || (window.vc = {}),
        ...
        void 0 === window.vc && (window.vc = {}), window.vc.ShortcodesBuilder = function(models) {
            ...
            buildFromTemplate: function(html, data) {
                ...
                return _.each($(html), function(block) {
                    ...
                    var model, id_param, shortcode = JSON.parse(decodeURIComponent(encoded_shortcode + "")),
                        $block = window.vc.$page.find("[data-model-id=" + shortcode.id + "]"),
                        params = _.isObject(shortcode.attrs) ? shortcode.attrs : {};
                    templateShortcodesHasId || (id_param = window.vc.shortcodeHasIdParam(shortcode.tag)) && !_.isUndefined(params) && !_.isUndefined(params[id_param.param_name]) && 0 < params[id_param.param_name].length && (templateShortcodesHasId = !0), model = window.vc.shortcodes.create({
                        id: shortcode.id,
                        shortcode: shortcode.tag,
                        params: this.unescapeParams(params),
                        parent_id: shortcode.parent_id,
            ...
            renderShortcode: function($html, model) {
                ...
                window.vc.last_inner = inner_html.html(), $("script", inner_html).each(function() {
                    var key, key_inline;
                    update_inner = ($(this).attr("src") ? (key = window.vc.frame.addInlineScript($(this)), $('<span class="js_placeholder_' + key + '"></span>').insertAfter($(this))) : (key_inline = window.vc.frame.addInlineScriptBody($(this)), $('<span class="js_placeholder_inline_' + key_inline + '"></span>').insertAfter($(this))), !0), $(this).remove()
        ...
        var Shortcode = Backbone.Model.extend({
            ...
            Shortcodes = Backbone.Collection.extend({
                ...
                _getShortcodeContent: function(parent, state) {
                    var params, models = _.sortBy(window.vc.shortcodes.where({
    ...
    function($) {
        ...
        vc.map = {}, vc.setFrameSize = function(size) {
            ...
            return values = _.extend({}, paramsMap.defaults, values), paramsDependencies = _.extend({}, paramsMap.dependencyMap), _.each(values, function(value, key) {
                if ("content" !== key) {
                    ...
                    if (!_.isUndefined(paramsDependencies[key])) {
                        ...
                        var rules, isDependedEmpty, dependedElement = paramsDependencies[key].element,
            ...
            var params = _.isArray(vc.getMapped(tag).params) ? vc.getMapped(tag).params : [];
            ...
            checkControlsPosition: function() {
                var window_top, new_position, element_height;
            ...
            addControls: function() {
                var shortcodeTag = this.model.get("shortcode"),
                    ...
                    compiledTemplate = vc.template(_.unescape(template), _.extend({}, vc.templateOptions.custom, {
                        evaluate: /\{#([\s\S]+?)#}/g
                    }));
            ...
            saveRowOrder: function(event, ui) {
                _.defer(function(app) {
                    ...
                    $rows.each(function(key, value) {
                        var row_data, model, prev_parent, current_parent, $el = $(this);
                        $el.is(".droppable") ? (row_params = {}, column_params = {
                            ...
                            parent_id: builder.lastID(),
                            params: column_params
                        }).render()) : (prev_parent = (model = vc.shortcodes.get($el.data("modelId"))).get("parent_id"), current_parent = $current_parent.closest(".vc_element").data("modelId") || !1, model.save({
                            order: key,
    ...
    function($) {
        ...
        window.vc.events.on("shortcodeView:updated", function(model) {
            ...
            addControls: function() {
                var parentShortcodeTag, allAccess, moveAccess, editAccess, parentAllAccess, parentEditAccess, data, parentName, shortcodeTag = this.model.get("shortcode"),
                    template = $(this.controls_selector).html(),
                    parent = vc.shortcodes.get(this.model.get("parent_id"));
                parent && (parentName = vc.getMapped(parent.get("shortcode")).name, parentShortcodeTag = parent.get("shortcode")), allAccess = vc_user_access().shortcodeAll(shortcodeTag), editAccess = vc_user_access().shortcodeEdit(shortcodeTag), parentAllAccess = vc_user_access().shortcodeAll(parentShortcodeTag), parentEditAccess = vc_user_access().shortcodeEdit(parentShortcodeTag), moveAccess = vc_user_access().partAccess("dragndrop"), data = {
                    name: vc.getMapped(shortcodeTag).name,
                    tag: shortcodeTag,
        ...
        window.InlineShortcodeView_vc_column = window.InlineShortcodeViewContainerWithParent.extend({
            ...
            customCssClassReplace: function() {
                var css_classes = this.$el.find(".wpb_column").attr("class"),
                    ...
                    class_match = !(!css_classes || !css_classes.match) && css_classes.match(css_regex);
    ...
    function($) {
        ...
        window.InlineShortcodeView_vc_tta_accordion = window.InlineShortcodeViewContainer.extend({
            ...
            getSiblingsFirstPositionIndex: function() {
                ...
                return first_shortcode && (order = first_shortcode.get("order") - 1), order
            },
            changed: function() {
    ...
    function($) {
        ...
        window.InlineShortcodeView_vc_tta_tabs = window.InlineShortcodeView_vc_tta_accordion.extend({
            ...
            sectionUpdated: function(model, justAppend) {
                var index, $element, html, tabAdded = !1,
                ...
                $tabEl.length ? (html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $tabEl.html(html)) : (html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $element = $('<li class="vc_tta-tab" data-vc-target-model-id="' + sectionId + '" data-vc-tab><a href="javascript:;" data-vc-use-cache="false" data-vc-tabs data-vc-target="[data-model-id=' + sectionId + ']" data-vc-container=".vc_tta">' + html + "</a></li>"), !0 !== justAppend && -1 < (index = _.pluck(_.sortBy(vc.shortcodes.where({
                    parent_id: this.model.get("id")
                }), function(childModel) {
            ...
            removeSection: function(modelId) {
                var $nextTab, $viewTab = this.$el.find('.vc_tta-tabs-container .vc_tta-tabs-list [data-vc-target="[data-model-id=' + modelId + ']"]').parent();
            ...
            updateTabsPositions: function($panels) {
                var $elements, tabSortableData, $tabs = this.$el.find(".vc_tta-tabs-list");
                $tabs.length && ($elements = [], tabSortableData = $panels.sortable("toArray", {
            ...
            updatePanelsPositions: function($tabs) {
                var $panels = this.getPanelsList(),
                    $elements = [],
                    tabSortableData = $tabs.sortable("toArray", {
    ...
    function($) {
        ...
        window.vc.ttaSectionActivateOnClone = !1, window.InlineShortcodeView_vc_tta_section = window.InlineShortcodeViewContainerWithParent.extend({
            ...
            refreshContent: function(noSectionUpdate) {
                var $controlsIcon, $controlsIconsPositionEl, parentParams, paramsMap, parentModel = vc.shortcodes.get(this.model.get("parent_id"));
            ...
            destroy: function(e) {
                var parentModel, parentId = this.model.get("parent_id");
                window.InlineShortcodeView_vc_tta_section.__super__.destroy.call(this, e), parentModel = vc.shortcodes.get(parentId), vc.shortcodes.where({
                    parent_id: parentId
    ...
    function($) {
        ...
        window.vc.cloneMethod_vc_tab = function(data, model) {
            ...
            buildTabs: function(active_model) {
                var active_el;
                active_model && (this.active_model_id = active_model.get("id"), this.active = this.tabsControls().find("[data-m-id=" + this.active_model_id + "]").index()), !1 === this.active_model_id && (active_el = this.tabsControls().find("li:first"), this.active = active_el.index(), this.active_model_id = active_el.data("m-id")), this.checkCount() || window.vc.frame_window.vc_iframe.buildTabs(this.$tabs, this.active)
            },
            checkCount: function() {
              /*nectar addition*/
    ...
    function($) {
        ...
        $(function() {
            ...
            window.vc && window.vc.events && ($undoControl = $("#vc_navbar-undo"), $redoControl = $("#vc_navbar-redo"), checkControls = function() {
                ...
                var newContent;
                $(this).is("[disabled]") || window.vc.undoRedoApi.isLocked() ? e && e.preventDefault && e.preventDefault() : (window.vc.undoRedoApi.lock(), newContent = window.vc.undoRedoApi.undo(), renderNewContent(newContent))
            }), $redoControl.on("click.vc-redo", function(e) {
                var newContent;
                $(this).is("[disabled]") || window.vc.undoRedoApi.isLocked() ? e && e.preventDefault && e.preventDefault() : (window.vc.undoRedoApi.lock(), newContent = window.vc.undoRedoApi.redo(), renderNewContent(newContent))
            }))
        })
    }(window.jQuery);